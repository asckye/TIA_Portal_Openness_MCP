...
namespace TiaMcpServer.Tests
{
    internal static class HardwareServicesTests
    {
        internal static void Run(Action<bool,string> check)
        {
            ...
            // 2.7.53 PLC protection gating (access level + master secret; official pages "Access level setting" / "Managing PLC Master Secret in PLCs")
            check(PlcProtectionLogic.Validate("read", "", "", "").Action == "read" && PlcProtectionLogic.Validate("SETACCESSLEVEL", "fullaccessincludingfailsafe", "", "").AccessLevel == "FullAccessIncludingFailsafe", "protection: actions and levels are case-insensitive, canonical names returned");
            check(Fails<ArgumentException>(() => PlcProtectionLogic.Validate("setAccessLevel", "None", "", "")) && Fails<ArgumentException>(() => PlcProtectionLogic.Validate("setAccessLevel", "", "", "")), "[sentinel] protection: None / empty access level refused");
            check(Fails<ArgumentException>(() => PlcProtectionLogic.Validate("protectMasterSecret", "", "", "")) && Fails<ArgumentException>(() => PlcProtectionLogic.Validate("setAccessPassword", "FullAccess", "", "")), "[sentinel] protection: password actions need a password");
            check(Fails<ArgumentException>(() => PlcProtectionLogic.Validate("changeMasterSecret", "", "old", "")) && PlcProtectionLogic.Validate("changeMasterSecret", "", "old", "new").Action == "changeMasterSecret", "protection: changeMasterSecret needs both passwords");
            check(Fails<ArgumentException>(() => PlcProtectionLogic.Validate("setAccessPassword", "NoAccess", "pw", "")) && Fails<ArgumentException>(() => PlcProtectionLogic.Validate("setAccessPassword", "FullAccessIncludingFailsafe", "pw", "")), "[sentinel] protection: no password exists for NoAccess / FullAccessIncludingFailsafe");
            check(PlcProtectionLogic.Validate("unprotectMasterSecret", "", "", "").Action == "unprotectMasterSecret" && PlcProtectionLogic.Validate("protectAllConfiguration", "", "", "").Action == "protectAllConfiguration", "protection: unprotect / protectAll accept an empty password");
            check(PlcProtectionLogic.PasswordLevelWarning("HMIAccess", "FullAccess") == null && PlcProtectionLogic.PasswordLevelWarning("HMIAccess", "HMIAccess") != null && PlcProtectionLogic.PasswordLevelWarning("NoAccess", "ReadAccess") == null, "protection: password level must be less strict than the selected level");
            check(PlcProtectionLogic.NeedsFullAccessPassword("NoAccess") && PlcProtectionLogic.NeedsFullAccessPassword("ReadAccess") && !PlcProtectionLogic.NeedsFullAccessPassword("FullAccess") && !PlcProtectionLogic.NeedsFullAccessPassword("FullAccessIncludingFailsafe"), "protection: levels above FullAccess need the FullAccess password (hardware compile rule)");
            check(PlcProtectionLogic.ExpectedMasterSecretStates("protectMasterSecret", true).Contains("WithPassword") && PlcProtectionLogic.ExpectedMasterSecretStates("unprotectMasterSecret", false).SequenceEqual(new[] { "None" }) && PlcProtectionLogic.ExpectedMasterSecretStates("read", false).Length == 0, "protection: expected master-secret states per action");
