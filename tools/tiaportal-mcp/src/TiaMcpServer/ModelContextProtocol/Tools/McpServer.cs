        ...
        public static async Task<ResponseBootstrap> Bootstrap()
        {
            try
            {
                ...
                var rules = new[]
                {
                    ...
                    "PLAN, DO NOT PROBE: before an unfamiliar call and after any correction from the user, run PreflightToolCall(name, argumentsJson) — it checks the arguments against the real signature and reports dryRun / confirm flags, precautions and session prerequisites without executing; fix the plan from that report, then call once. Every listed tool description ends with a worked Example.",
