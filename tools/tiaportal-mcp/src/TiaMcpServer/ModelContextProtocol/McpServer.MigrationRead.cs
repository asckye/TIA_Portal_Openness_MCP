...
namespace TiaMcpServer.ModelContextProtocol
{
    ...
    {
        [McpServerTool(Name = "ListUnifiedGlobalScripts"), Description("[L2][HMI-Unified][READ-ONLY]Page global script module names only. Names are NOT proof of body acquisition. Exact expectedProject and HMI path required. Resume with identical arguments and nextCursor.")]
        ...
        [McpServerTool(Name = "ReadUnifiedGlobalScript"), Description("[L2][HMI-Unified][READ-ONLY]Native-export ONE exact global module to temporary files, return original JS/YAML, functions, parameters, bodies, global definitions, imports and unresolved tag expressions. Never imports, saves or executes scripts. Follow nextCursor; inspect dataComplete and failures.")]
        ...
        [McpServerTool(Name = "ReadUnifiedScreenBranch"), Description("[L2][HMI-Unified][READ-ONLY]Read an exact screen/item branch with continuation. screenPath is /Group/Screen with URI-escaped names. branchJson is [{property:'Interface'},{name:'Speed',key:'PropertyName'},{property:'Dynamizations'}] encoded as valid JSON; property, attribute, index and exact name selectors only. [] reads the selected subtree, never Parent. No writes or arbitrary method invocation.")]
        ...
        [McpServerTool(Name = "ReadUnifiedLibraryType"), Description("[L2][HMI-Unified][READ-ONLY]Read/native-export ONE exact project-library faceplate or script type AND version. typePath=/Folder/Type, version exact (never default). Direct dependency identifiers only; never opens edit mode, instantiates or walks the entire library. Original WinCCML files and path-addressed internal values; unsupported export is an explicit gap.")]
        ...
        [McpServerTool(Name = "ListUnifiedLibraryFolder"), Description("[L2][HMI-Unified][READ-ONLY]Page only direct type/folder names in ONE exact project-library folder, for locating a referenced script/faceplate. No recursive search or default-version substitution. / selects root. Inspect returned folders before choosing the next exact path.")]
        ...
        [McpServerTool(Name = "ReleaseUnifiedReadCursor"), Description("[L2][HMI-Unified][READ-ONLY]Release one collection cursor and its temporary export files. Never saves or closes TIA Portal. Cursors otherwise expire after 30 minutes without use or on project switch/server restart.")]
