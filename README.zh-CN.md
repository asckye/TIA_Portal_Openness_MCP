> Release build / 完整发布包：**v2.7.12（V20 + V21，文件版本 2.7.12.0）**。包含运行程序、原版配置、手册、模板、源码与测试，见 [本版说明](docs/releases/v2.7.12.md)。新接口的真实工程验收尚未完成。每次正式发布均须遵循 [完整打包流程](docs/RELEASE_WORKFLOW.md)。
...
本仓库由 **asckye** 独立维护，仅使用 `master` 开发 TIA Portal **V20 / V21**，构建、问题反馈和 Release 发布都在本仓库进行。修改和 PR 提交到本仓库的 `master`。

本软件包含原项目及贡献者以 MIT 许可发布的成果。原作者版权、许可证和来源说明保留于 [NOTICE.md](NOTICE.md) 与 [LICENSE](LICENSE)；独立仓库及新的导入提交不表示全部代码为当前维护者原创。

请勿提交客户工程、连接密钥、私人配置或构建缓存。协作流程见 [CONTRIBUTING.md](CONTRIBUTING.md)，安全问题报告见 [SECURITY.md](SECURITY.md)。
