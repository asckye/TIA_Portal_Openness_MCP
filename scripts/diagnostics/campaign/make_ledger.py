...
o("GetCrossReferences", PASS, RERUN + "：filter 为空按 AllObjects；非法 filter 列出合法值；" + D281 + "：Override 重导 5 块未编译后查旧 IDB，查询返回后 TIA Portal V21 自行退出（事件 3000 SELF，-1；handoff §4 退出点 ⑧）→ 2.9.0 删除类工具默认不查，2.9.1 PLC 有未编译块即拒绝并点名；" + D291 + "：项目1 MCP_STD 已编译时正常返回；Override 重生成后未编译 → refused … Main, MCP_SimLogic；CompileSoftware 后恢复，TIA 未退出")
o("DeletePlcBlock", PASS, "2.7.46 真机通过；" + D281 + "：干跑里的交叉引用查询让 TIA 退出 → 2.9.0 新参数 crossReferences（默认 false），响应标 crossReferenceQueried=false；" + D291 + "：默认干跑秒回 crossReferenceQueried=false；crossReferences=true 在未编译 PLC 上被门挡住（警告缺拒绝原因，源码已补）")
