...
- 早期记录描述历史实现，早期版本分支不属于当前维护范围。

## [2.7.3] - 2026-09-13

- 独立项目使用标准版本号发布：Release 标签为 `v2.7.3`，完整包为 `TIA_MCP_Delivery_v2.7.3_20260913.zip`；后续按 `v2.7.4` 等递增。
- 新增全局脚本、变量成员分页、精确库类型/面板实例、页面对象分支等八个只读采集接口，保留原始脚本、来源证据及明确的缺口/续采状态。
- V20/V21 程序统一为 FileVersion `2.7.3.0`，随包附带原版配置入口、依赖、手册、模板、源码及测试，不需要旧版补文件。
- 每次发布使用统一完整打包流程，校验源码输入、两版 EXE、运行依赖及 ZIP 内全部文件。
- 本地验证与真实工程验收分开报告；新接口在真实博途工程中的迁移完整性仍待验收，见 [本版说明](docs/releases/v2.7.3.md)。

## [2.7.2-asckye.6-defects.1] - 2026-09-13

- 修复只读事件查询创建事件及失败状态被覆盖、空文本规范化与部分写入回执、编译根摘要掩盖子错误、属性路径与 null 混同、原生导出覆盖失败。
- 增加精确事件/动态对象读删、空组删除、完整画面路径、有范围说明的快照及已保存工程原生归档。
- HTTP JSON 使用严格 UTF-8；增加请求错误诊断和鉴权 readiness，保留既有响应路由、HMI 递归与语言保护。
- V20/V21 FileVersion 2.7.2.6。离线与本机 EXE 验证通过；未替换虚拟机、未进行真实工程验收。
- 详见 [修复与现场验收说明](docs/releases/v2.7.2-asckye.6-defects.1.md)。


## [2.7.2-asckye.5-multilingual.1] - 2026-09-13

- 修复 Unified 多语言写入：按 Language.Culture.Name 唯一匹配，取消首语言兜底，写后验证目标及非目标语言。
- 明确省略 text 保持、空字符串清空、null/非字符串失败的契约，保留空白和 HTML 内容。
- 新增 ReadUnifiedHmiTexts；GetObjectProperty 读取 MultilingualText 返回语言代码和文本，避免 Culture.Parent 循环序列化。
- CallTool 分离 bridgeSuccess 与 operationSuccess；业务失败不再被外层成功掩盖，未知结果明确为 null。
- 更新 V20/V21 runtime；FileVersion 2.7.2.5，ProductVersion 2.7.2-asckye.5-multilingual.1。
- 离线回归 328 通过、0 失败、0 跳过；V20/V21 均构建通过。虚拟机服务尚未替换，修复版实际 TIA 验收未执行。
- 部署及验收详见 docs/TIA_MCP_多语言修复与验收报告.md；逐文件哈希见 manifest/multilingual-build.json。

## [Unreleased] - 写 Unified JS 脚本不再赌上整个博途进程

### 配置方式

- **恢复原版客户端管理方式（2.7.2-asckye.4）。** 移除自定义 HTTP 启动器、
  本地配置保存和命名管道停止控制，保留 `配置MCP.bat` / `配置MCP-v20.bat`、
  STDIO、手动 HTTP 与原有预热流程。HTTP 响应路由和 HMI 子文件夹查找修复继续保留。
  原版 `.bat` / `.cmd` 统一为 UTF-8 无 BOM、Windows CRLF 换行，保留原有文字与执行逻辑。

### 修复

- **HTTP 请求超时后持续失联、重复请求 ID 导致响应错配。** HTTP 服务现在只有一个持续读取
  MCP 响应的任务；转发使用独立内部 ID，返回时恢复客户端 ID。240 秒等待上限包含排队时间，
  超时移除等待者并丢弃迟到响应，不再创建遗留读取任务。流结束或服务退出时清理所有等待请求。
  修复 .NET Framework 下 `WWW-Authenticate` 标头写入异常，保留 Bearer / X-API-Key 鉴权。
  增加直接加载编译 EXE 的 HTTP 回归测试；健康检查增加 `fileVersion` 便于确认运行版本。
...
- **`CompileAndDiagnoseHmi`**（#24（原项目历史记录，来源见 NOTICE.md））
...
- **`GetPlcTagTables` 恒返回空列表**（#22（原项目历史记录，来源见 NOTICE.md））。
...
- GitHub Releases / v0.0.29（原项目历史记录，来源见 NOTICE.md） 提供 **`TIA_MCP_完整交付包_v0.0.29.zip`**：与仓库根目录内容一致（含双版本 exe），打包时排除 `.git` 与 `TiaMcp_Output/`。
...
- 公开仓库 bulaofen0036-coder/TIA_MCP_260514（原项目历史记录，来源见 NOTICE.md） 从 `TIA_MCP_交付包_20260512_151308` 全量刷新至 `TIA_MCP_交付包_20260525_V20S7DCL_184330`。
