# 官方 Openness API 覆盖清单（V21，2.7.35）
...
370 个"完全未触及"的类型里（2.7.34 为 406，2.7.33 为 428，2.7.32 为 490，2.7.31 为 513，2.7.30 为 535，2.7.29 为 556，2.7.27 为 599，2.7.26 为 626，2.7.25 为 885；差额是登记为动态覆盖的 322 个 Unified 类型与 2.7.30 强类型封装的硬件网络类型），`*Composition` / `*Association` / `*EventArgs` / `*Exception` / `*Result` / `*Info` 这类集合与结果壳子 333 个，Unified `UI.Events`（96）与 `UI.Parts`（85）经 `GetCompositionInfos` / 泛型 `Create<T>` 动态覆盖而词法不可见（2.7.26 起已从"未触及"移入动态覆盖栏）。去掉这三类后，**没有专用封装的功能类型为 139 个 / 637 个成员**（2.7.34 为 162 / 695，2.7.33 为 178 / 735，2.7.32 为 230 / 896，2.7.31 为 239 / 929，2.7.30 为 253 / 1,006，2.7.29 为 267 / 1,067，2.7.27 为 288 / 1,185，2.7.26 为 308 / 1,286；2.7.26 起所有行统一用审计脚本的壳子规则重算），其中核心程序集（Base / Step7 / WinCC / WinCCUnified）29 个 / 101 个，`WinCC.Extension` 2 个 / 11 个，选件包（SiVArc / Startdrive / DCC / Test Suite / SafetyValidation / Teamcenter / CFC）108 个 / 525 个。
...
| Step7 | 5 | 42 | **2.7.35 ④-② 已封装**Step7 收尾（`ManagePlcExternalSources`：`PlcExternalSource` / `PlcExternalSourceGroup` / `PlcExternalSourceSystemGroup` / `PlcExternalSourceUserGroup`；`ReadPlcSystemGroups`：`PlcSystemBlockGroup` / `PlcSystemTypeGroup`；`ReadPlcTagTableConstants`：`PlcConstant`；`ExchangePlcAlarmTextListsXlsx`：`PlcAlarmTextListProvider`；`ManagePlcTableEntries`：`PlcTableCommentEntry` / `PlcWatchTableEntry` / `PlcForceTableEntry`（强制表只读）；`ExportPlcProDiagInfo`：`CodeBlock.ExportProDIAGInfo`；类型化的 `WatchTableAccessRule` / `ForceTableAccessRule`、`OpcUaCommunicationGroup` / `NamespaceAccessRestriction`、`AlarmClassExportImportResultMessage` / `SupervisionSettingsExportImportResultMessage`、`CodeBlockLibraryType` / `PlcTypeLibraryType` / `PlcDocumentLibraryType`、`PlcTypeUserGroup` / `PlcTagTableUserGroup` / `PlcWatchAndForceTableUserGroup`；2026-09-19 真机验证通过）。**2.7.34 ④-①** 软件单元与 `PlcSoftware` 小服务已真机验证。剩余 ④-③ 工艺对象映射：`TOMapping`、`AxisEncoderHardwareConnectionInterface`（19）/ `TorqueHardwareConnectionInterface`（14）、`TechnologicalInstanceDBGroup`、`TechnologicalParameter` |
...
## 统计表（脚本生成，2.7.35 源码；"动态覆盖"栏来自登记表）
...
| Step7 | `Siemens.Engineering.SW.TechnologicalObjects` | 8 | 2 | 0 | 0 | 6 | 27 | 6 |
...
| WinCC | `Siemens.Engineering.Hmi.Tag` | 9 | 5 | 0 | 0 | 4 | 28 | 15 |
