...
| Unified 原生交换（2.7.28） | `ExchangeUnifiedTags`、`ExchangeUnifiedScriptModules`、`ImportUnifiedOpcUaAlarms` | 导出要求新目录、每个原生文件哈希；导入要求现有目录并核对期望名/模块名；`OpcUaAlarm` 只在 OPC UA 连接上存在；默认预览，无保存/编译/下载 |
