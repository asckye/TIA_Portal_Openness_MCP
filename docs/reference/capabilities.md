...
本文说明 2.9.1 引擎的能力与缺口：最新的工具族在前，按版本倒序追加，2.7.14–2.7.15 的基础表格保留在后。静态清单 453 项（7 个大类，见 `ListToolCategories` 与 [工具矩阵](tool-matrix.md)），默认 lite 暴露 59 项。工具数量不表示覆盖全部 API——对照官方 V21 PublicAPI 的逐类型记分板在[官方 API 覆盖清单](openness-coverage.md)（2.7.42：核心程序集 Base / Step7 / 经典 WinCC / WinCC Unified / Safety 与全部选件包 SiVArc / Startdrive / DCC / SafetyValidation / Test Suite / Teamcenter / CFC 的功能类型缺口均为 0）。原生方法按本机官方 V20/V21 PublicAPI 对照实现，每个调用的成员在构建时做程序集形状检查；**真机验收状态按版本分段记录**——每段末尾的"真机"句说明哪些在 V21 参考工程 `AutomaticDipCoatingMachine` 上跑过、哪些只有形状检查（选件包无许可、经典 HMI 无工程），不能混同。

## 2.9.1 交叉引用查询的护栏

`GetCrossReferences` 与 `crossReferences=true` 在 PLC 有任一未编译块（`IsConsistent=false`）时拒绝并点名，先 `CompileSoftware`——维护者真机：Override 重导未编译后查旧 IDB 的引用，TIA Portal V21 自行退出（事件 3000 SELF，退出码 -1）。详见 `docs/releases/v2.9.1.md`。

## 2.9.0 删除类工具的交叉引用改为显式开关

`DeletePlcBlock` / `DeletePlcType` / `DeletePlcTagTable` 默认不再调用 `CrossReferenceService.GetCrossReferences`（维护者真机：干跑时让 TIA Portal V21 整个退出），新参数 `crossReferences=true` 才查；响应 `crossReferenceQueried` 说明是否查过。`GetCrossReferences` 描述标明风险。详见 `docs/releases/v2.9.0.md`。
...
## 2.7.62 更新器只在线 + 客户端卡片英文名

`Update-Engine.ps1` 去掉离线 `-ZipPath` / `-SkipHashCheck`：只从 GitHub Release 在线取 ZIP + `.sha256`（必须有）并校验，github.com 不通就失败并说明；`CheckForUpdate` 的步骤改为三步。配置器 12 张卡片一律英文名（Qwen、Yuanbao、Zhipu GLM、Qwen Agent；种类 CLI / Desktop / IDE）。详见 `docs/releases/v2.7.62.md`。

## 2.7.61 配置器：本机客户端检测 + 千问工作助理卡片

配置器逐张卡片检测客户端在本机的痕迹（配置目录 / PATH 可执行文件 / 安装目录 / 卸载项），副标题显示已检测 / 未检测到，写入位置按当前用户解析，同一 EXE 在任何电脑上都写对位置；新卡片“千问工作助理”写 `~/.qwen-agent/mcp.json`（`url` + Bearer 头，不走 OAuth）。12 张卡片。详见 `docs/releases/v2.7.61.md`。

## 2.7.60 派生示例占位符修正

2.7.59 真机：`FindTools` 里无人工示例的工具的派生示例把 `devicePathJson` 显示成 `"["`、`chartPath` 当成主机路径。规则重排：`*Json` 先按名字给 `[]` / `{}`，`*Path` 按名字区分主机路径与工程内对象路径（`<Folder/Name>`），`e.g.` 值只取纯值。详见 `docs/releases/v2.7.60.md`。
...
维护者 2026-09-21：规范 AI 对**所有**工具的调用、不再试错。本版四件事都由引擎自动做、对 453 个工具生效：① 参数描述里的备选值 / C# 默认值 / 示例值写进每个工具的 `inputSchema`（`enum` / `default` / `examples`；67 个 enum 落在 51 个工具上，解析只认参数自己那句话里独立成段的列表，真机核对过误判）；② 任何失败调用自动附 `meta.preflight`（缺参 / 参数名 / 类型 / `allowedValues` / 前提 / 示例 / 下一步），成功响应不动；③ `ParameterVocabulary` 79 条通用参数描述填进没有 `[Description]` 的参数（1488 → 1392 无自身描述，1002 由词汇表覆盖），手写批次 1 覆盖 14 个常用工具，`tools-list.json` 的 `callDiscipline` 统计 + 构建门禁止回升；④ 示例覆盖 453 个（80 条人工 + 按签名派生）。新增 `GetRecipe(topic)`：12 条真机跑通的多步序列，每步精确调用，构建时校验。工具 453，lite 59。详见 `docs/releases/v2.7.58.md`。
...
2.7.50 真机：`ManagePlcTableEntries deleteTable` 删掉了 `MCP_WT_1` … `MCP_WT_5`；`SetWatchTableModifyValue` 的 SimaticML 往返被 TIA 拒 "'set_ModifyIntention' is not supported … read-only"——行里不再写 `ModifyIntention`，导出带出来的也在导入前剥掉；PLCSIM Advanced 8.0 的 `IInstance.CommunicationInterface` 只读且没有任何 `Set…` 方法，手册明说 "To set the network mode, refer to the NetworkMode section"——接口选择是全局的 `SimulationRuntimeManager.NetworkMode`（`TCPIPMultipleAdapter` / `TCPIPSingleAdapter` / `Softbus`，有实例在跑时 API 拒绝），`communicationInterface` 现在映射到它并在 `data.communicationInterfaceRoute` 回报前后模式，`ReadPlcSimAdvancedInstances` 回报 `api.networkMode` 与 `managerMembers`；`UploadStationFromPlc` 对 MAC 明说 `ConfigurationAddressComposition.Create` 只收 IP（未下载过的 PLCSIM 实例没有 IP）。详见 `docs/releases/v2.7.51.md`。
...
2.7.46 / 2.7.47 部署后 43/45 行 🔁 转为通过；刻意绕开的工程级工具与工艺对象也跑了（`TO_PositioningAxis 5.0` 可建，crash ⑨ 只在 6.0）；经典画面尺寸不一致 = crash ⑩，`ImportHmiScreen` 先比对面板分辨率；新增 `ManageOpcUaInterface`（read / delete）；`ManagePlcSimAdvancedInstance` 支持 `communicationInterface=TCPIP`。详见 `docs/releases/v2.7.48.md` 与[真机台账](real-machine-ledger.md)。

## 2.7.47 宽松绑定

`CallTool` 桥接接受对象形式的 `argumentsJson`、对象/数组形式的 `*Json` 参数、字符串形式的数字/布尔，枚举大小写按拒绝信息重试一次（`Meta.bridgeNormalizedArguments` 回报）。

## 2.7.46 全量真机重测（空工程 `项目1`，447 个工具各跑一遍）

逐工具结论见[真机台账](real-machine-ledger.md)。本版修的 23 处见 `docs/releases/v2.7.46.md`；新守卫：Unified 面板版本主号须等于 Portal 主版本（crash ⑧），`ManageTechnologyObject create` 标明 crash ⑨。
