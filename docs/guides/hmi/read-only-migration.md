...
如博途无弹窗退出，在虚拟机完整包目录的 CMD 执行 `scripts\diagnostics\Collect-TiaExitEvidence.cmd`，只读提取最近 24 小时内最多 100 条应用崩溃/.NET/WER 事件、当前博途进程列表和本版原生导出日志。输出位置会显示在控制台；空事件不等于没有异常。先在本地检查后再分享，日志和事件可能包含私有路径。该命令不连接、启动或关闭博途。
