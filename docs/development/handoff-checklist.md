...
- 仓库 `master` = `origin/master`，最后一次发布 **v2.7.59**（tag、三条工作流全绿，ZIP `TIA_MCP_Delivery_v2.7.59_20260921.zip` 已上传；`Release.ps1` 一条命令全程自动）。453 个工具（lite 59），离线 2453，形状 V20 2805 / V21 3097；参数 2204 个，1310 个有自己的描述、其余 894 个词汇表覆盖，enum 提示 185。虚拟机上跑的是 **2.7.58**（2026-09-21 `Update-Engine.ps1` 在线更新；安装目录 `C:\Users\SIEMENS\Desktop\TIA_MCP_Delivery_v2.7.57_20260921`，虚拟机有外网）。以后部署：先停引擎，在安装目录 `scripts\operations\Update-Engine.ps1`（在线）或 `-ZipPath <ZIP>`，**更新完要手动重启引擎**（8765 端口在重启前无响应），`Bootstrap` 看版本；出问题 `-Rollback`。
...
- 工程已于 2026-09-21 05:48 UTC 保存（MCP_STD + 程序、两台 CPU 的保护设置、监控表两行）。
...
2. ~~**2.7.57 / 2.7.58 / 2.7.59 部署后**~~ 都通过（2026-09-21）。**2.7.60 / 2.7.61 部署后**：虚拟机上 `FindTools {query:"dcc chart"}` 的派生示例应为 `{"devicePathJson":"[]","itemPathJson":"[]"}`、`chartPath` 为 `<Folder/Name>`；宿主机上换新的 `TiaMcpConfigurator.exe`，启动日志应列出 12 张卡片的检测结果，千问工作助理卡片写 `~/.qwen-agent/mcp.json` 后完全退出该应用再打开。虚拟机有外网：先停引擎，`Update-Engine.ps1`（在线），更新完手动重启引擎。
...
- **一键发布**：`powershell -NoProfile -ExecutionPolicy Bypass -File scripts/build/Release.ps1 -Version X.Y.Z -Summary "…"`（2.7.56 之后入库，见 handoff §4 第一条与[发布流程](release-workflow.md)）。它自动改版本号 8 处（`.claude-plugin/plugin.json`、`tools/mcp-configurator/Configurator.cs` 两行、`TiaMcpServer.V20.csproj` / `V21.csproj` 各三行、`docs/README.md` 的"当前发布说明"链接、`docs/reference/capabilities.md` 首段、`docs/development/roadmap.md` 标题）；脚本拒绝启动的两样要先手写：`CHANGELOG.md` 顶部条目 + `docs/releases/vX.md`（`Validate-Bundle -Strict` 要求 CHANGELOG 最新条目 = csproj 的 InformationalVersion）；`capabilities.md` 新段与 `roadmap.md` §5 条目仍手写（脚本只提醒）。
...
- 提交 / 推送 / CI / tag 都由 `Release.ps1` 做，失败时照它的顺序手工补：`Release X (1/3)` 源码 + 版本 + 文档；`(2/3)` `runtime/v20/TiaMcpServer.exe`；`(3/3)` `runtime/v21/TiaMcpServer.exe` + `TiaMcpConfigurator.exe` + `manifest/*` + `docs/reference/tool-matrix.md`。然后 `python scripts/build/Package-Release.py --git "<git.exe>"` 本地干跑（要求工作区干净），`git rev-list --reverse --first-parent origin/master..master` 逐个 `git -c http.postBuffer=157286400 push origin <sha>:refs/heads/master`（一次推 30 MB 曾 408），CI 抓工作流页的 `aria-label`（REST API 未登录会 403 限流；短 sha 查不到），两绿后 `git tag -a vX -F <utf8 文件>` + `git push origin vX`，等 "Publish complete release" 成功、发布页出现 ZIP，再在 handoff §1 改现状、`handoff-history.md` 加条目，单独提交。
