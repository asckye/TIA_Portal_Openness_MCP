...
- 提交 / 打包 / 推送 / CI / tag / 上传都由 `Release.ps1` 做（2.8.1 起），失败时照它的顺序手工补：一次提交 `Release X: <summary>`（二进制在 `.gitignore` 里，不会进来）；`python scripts/build/Package-Release.py --git "<git.exe>"`（树 + 本机 `runtime/` + `TiaMcpConfigurator.exe` → `bin-build/releases/vX/`）；`python scripts/checks/Verify-ReleaseAsset.py <zip>`；`git push origin master`；等 `validate-bundle` / `offline-checks`（`Release.ps1` 带令牌查 API）；`git tag -a vX -F <utf8 文件>` + `git push origin vX`；`powershell -File scripts/build/Publish-Release.ps1 -Version X`（草稿 → 上传 → 回读 → 发布；令牌来自 GCM）；等 "Verify published release" 绿；再在 handoff §1 改现状、`handoff-history.md` 加条目，单独提交。**clone 到新机器后先跑 `Build-Release.ps1`**，否则没有 `runtime/` 与 `TiaMcpConfigurator.exe`。
