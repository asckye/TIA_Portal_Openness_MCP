...
状态（2.7.48）：本表各项已在 2.7.18–2.7.36 间落地（对应版本见 §5 各段与 CHANGELOG），本表保留为当时的优先级依据；未做的只剩 §2.2 的无 API 项与 §2.0 阶段 6 的选件包。
...
- `CrossReferenceGuardLogic`：PLC 有未编译块就拒绝交叉引用查询并点名（真机：Override 重导未编译 + 查询 → TIA 自退，事件 3000 SELF / -1）。

## 5.0 2.9.0 已完成

- 删除类工具（块 / UDT / 变量表）的交叉引用查询改为 `crossReferences` 显式开关，默认不查（真机：该查询让 TIA 退出）；`GetCrossReferences` 描述标明风险。
...
- 派生示例占位符规则修正（2.7.59 真机发现 `"["` 与 `chartPath` 主机路径）。
...
- 2.7.54 真机：singleStep 模式切换生效但只推进 1 周期——按手册改为每周期等待 `Freeze`（`StepCycles`），优先 `SingleStep_C`；站上载在 PLCSIM 实例上被 TIA 拒（不支持）；F-CPU GoOnline 文案通过。在线族收口。

## 5.0 2.7.54 已完成

- 2.7.53 真机：在线族在标准 CPU `MCP_STD` + PLCSIM Advanced 上全链走通（下载 / 上线 / 比较 / 标签读写 / 场景）；F-CPU 下载是 Openness 规则拒绝；修 singleStep 枚举名、PLCSIM 接口重复歧义、`GoOnline` 内层异常。
...
- `CallTool` 宽松绑定（对象形式 argumentsJson、`*Json` 参数给对象/数组、字符串数字/布尔、枚举大小写重试）。
