...
| 4 | **Step7**（`Siemens.Engineering.SW`） | 44 类型 / 140 成员 | **进行中**。④-② Step7 收尾——**2.7.35 完成**：`ManagePlcExternalSources`（`PlcExternalSource(Group/UserGroup/SystemGroup)`）、`ReadPlcSystemGroups`（`PlcSystemBlockGroup` / `PlcSystemTypeGroup`）、`ReadPlcTagTableConstants`（`PlcConstant`）、`ExchangePlcAlarmTextListsXlsx`（`PlcAlarmTextListProvider`）、`ManagePlcTableEntries`（`PlcTableCommentEntry` / `PlcWatchTableEntry` / `PlcForceTableEntry`）、`ExportPlcProDiagInfo`（`CodeBlock.ExportProDIAGInfo`）+ 类型化的访问规则 / OPC UA 通信组与命名空间限制 / 报警类与监控设置结果消息 / 库类型子类 / 用户组（Step7 未封装 28 / 100 → 5 / 42；真机待验）。④-① 软件单元与 `PlcSoftware` 小服务——**2.7.34 完成并真机验证**：`ReadPlcSoftwareUnits` / `ManagePlcSoftwareUnit`、`ManagePlcDocuments`、`ReadPlcChecksums`、`ReadPlcObjectFingerprints`、`ManagePlcBlockWriteProtection`、`ManageProjectCompilationSettings`、`ReadDeviceItemChannels includeLinkedTags`、V21 `ProcessImageProvider`。剩余 ④-③ 工艺对象映射 `TOMapping` / `AxisEncoderHardwareConnectionInterface` / `TorqueHardwareConnectionInterface` / `TechnologicalInstanceDBGroup` / `TechnologicalParameter`（5 / 42） |
