...
## 1. 现状（2.7.32 已发布并真机验证）

- 最新 tag `v2.7.32`（2026-09-18，本机构建并发布；上一个 `v2.7.31` 同日）。
...
1. ~~部署 2.7.32 到虚拟机并真机验证~~ **2026-09-18 完成**（结果见 `docs/releases/v2.7.32.md#真机结果`）。**2.7.33 先做这些引擎修补**（与 ③-④ 同一个发布）：
   - `ManageSyslogServers scope=project create`：真机上 `SyslogServerComposition.Create(name)` 抛无消息的 `NonRecoverableException` 并让 TIA Portal V21 进程退出（两次复现，`ListPortalProcessProjects` 从 1 变 0）。默认拒绝实做（只留预览 + 描述里写明），除非找到前置条件（可能与工程来源 V17 升级或"安全设置"节点缺失有关；官方没有专页）。
   - `ManagePlcCertificate`：`template` 加进免 `certificateId` 的动作列表（现在只有 create / import / unassign）。
   - `RequireUmacDevice` 报错文案：`UmacDevice` 在设备（`ET 200SP station_1`，`itemPathJson=[]`）上，不在 CPU 项；文案改成 "the Device or the DeviceItem that owns the UMAC configuration"。
   - `CheckLibraryUpdates`：`messageParts` 的 `KeyValuePair<string,string>` 直接渲染为 `{Key: Value}`（现在经 `EngineeringScalarProperties.Read`，1539 条 = 2.2 MB）。
   - 多 TIA 实例下的自动改绑：两个 TIA 进程时 `IsProjectNull()` 的自愈走 `ConnectPortal()` 重挑进程，引擎在两次调用之间从 `AutomaticDipCoatingMachine` 跳到了 `TESL_DU1_30000_SICAR4.0_CH_V17_V21`；写工具应在动手前核对 `_project.Name` 与上次显式 `AttachToOpenProject` 的名字一致（Unified 读工具已有 `expectedProject` 模式），或自愈只在单进程时进行。
   - 未验证项留给有条件时：工程级 syslog `update` / `delete` / `assignModule`、CPU 级 `createServer` / `update` 实做、`importFromServer` / `synchronize`（需要 UMC 服务器）、`import` 带密码、匿名用户实做。
