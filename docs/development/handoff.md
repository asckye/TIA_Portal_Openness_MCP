...
- 最新 tag `v2.7.32`（2026-09-18，本机构建；上一个 `v2.7.31` 同日）。
- 工具 389 个、默认 lite 56；离线套件 1620 项；引擎 API 形状检查 V20 1403 / V21 1500（`Build-Release.ps1` 第 73 行硬编码这两个数）。
- 审计（V21）：类型 1,217 = 专用引用 314 / 仅类型名 88 / 动态覆盖 325 / 完全未触及 490；未封装功能类型 230 / 896（Base 52 / 161、Step7 44 / 140、经典 WinCC 24 / 59、`WinCC.Extension` 2 / 11、选件包 108 / 525，Unified 0，Library 0，Security 0，Umac 0）。
- 阶段 3 ③-③ 用户管理与安全（2.7.32）：`ManageSyslogServers`（工程级 `SyslogServerProvider` + CPU `SysLogConfigurationManager`）、`ManagePasswordPolicy`（UMAC / PLC / legacy PLC 三套）、`ManageUmcUsers`（offline 与服务器导入的 UMC 用户 / 组、`UmcServerConfigurator`）、`ManagePlcCertificate` 的 `template` / SAN / 密码导入、`ManageProjectUserManagement` 的匿名用户激活；2.7.31 真机的四处库缺陷已修（`LibraryRef` 标签、null `TypeFolder` 守卫、`UpdateCheck` 预拒绝、用户库限定文案）。**全部未真机验证。**
...
1. **部署 2.7.32 到虚拟机并真机验证**（替换 `runtime/v21/TiaMcpServer.exe` 后**重启服务**；先 `python scripts/diagnostics/Probe-McpServer.py call GetState` 看 `server: TiaMcpServer 2.7.32.0`，再 `call AttachToOpenProject '{"projectName":"AutomaticDipCoatingMachine"}'`；非 lite 工具一律 `bridge <Tool> @args.json`，输出前设 `PYTHONIOENCODING=utf-8`）：
   - 库缺陷修复验证：工程库 `ReadLibraryOverview`（期望 `library.name=ProjectLibrary` + `project`，26 个顶层文件夹 PLC / LGF / LBC / _SICAR_Types_PLC / LSafe / …，`maxItems` 给 40）、`CheckLibraryUpdates` 两种 mode（工程库正常路径）、`SynchronizeLibrary` 三种预览（`updateProject` 范围 `+S1-K1`、`harmonizeProject`、`cleanUp`）、`CompareLibraries`（工程库 vs `Buttons-and-Switches`，先 `ManageGlobalLibrary openInfo`；跑通后把登记表 `LibraryCompareResult*` 的 `verified` 改成真机证据）、系统库 `ReadLibraryOverview includeTypes=true`（期望 `typeFolder=null` + `typeFolderNote`）、`CheckLibraryUpdates libraryName=Buttons-and-Switches`（期望调用前 `ArgumentException`，之后 `GetState` 里 `snapshotReadsBlocked=false`）、`ManageGlobalLibrary close` 对系统库（期望 "exists only on UserGlobalLibrary" 文案）。
   - `ManagePasswordPolicy read`（三套可用性；虚拟机上 UMAC 策略是 8 / 1 / 0 / 5 / 60 / 7、大小写 true、老化 false）→ `update target=umac propertiesJson={"MinimumLength":8}` 同值写回；`target=plc` / `legacyPlc` 先看 `available`。
   - `ManageSyslogServers`：`scope=project read`（期望空表或现有服务器）→ `create name=mcptmpsyslog2732 propertiesJson={"Address":"10.0.0.250","Port":6514,"Tls":true,"Comment":"tmp"}` → `update`（Comment）→ `assignModule devicePathJson=["ET 200SP station_1"] itemPathJson=["+S1-K1"]` → `unassignModule` → `delete confirmDelete=true`；`scope=plc read`（CPU `+S1-K1`：`EnableSystemLogging` / `TransportProtocol` / 三个动态属性）；PLC 级 `createServer` / `update` 只做预览（会改 CPU 组态）。
   - `ManageUmcUsers`：`kind=server read`（`umcServer` 无标量、`attributeNames`）与 `checkConsistency`（无 UMC 服务器，看 TIA 怎么答）；`kind=user createOffline name=mcptmpumc2732` → `read` → `rename` → `deactivate` / `activate` → `assignRole roleName=<一个自定义角色名，先 ReadProjectUserManagement customRoles>` → `unassignRole` → `delete`；`kind=group` 同一串（`createOffline` 走 `CreateOfflineUmcUserGroup()+SetName`，看 `nativeNameBeforeSetName`）。`importFromServer` 无服务器不测。
   - `ManagePlcCertificate`：`template usage=Tls`（`+S1-K1`，看默认 Signature / ValidFrom / ValidUntil / SAN）→ `create usage=Tls propertiesJson={"SubjectCommonName":"mcptmp2732"} subjectAlternativeNamesJson=[{"type":"Dns","value":"mcptmp.local"}] dryRun=false` → `list`（`enableGlobalCertificatesStore`）→ `delete certificateId=<新 id>`。
   - `ManageProjectUserManagement activateAnonymousUser` / `deactivateAnonymousUser` **只做预览**（匿名用户当前 Active，`ReadProjectProtection` 显示；停用可能让无密码客户端被锁在外面）；`ReadProjectUserManagement category=roleDeviceRights name=<系统角色> devicePathJson=["ET 200SP station_1"] itemPathJson=["+S1-K1"]` 看 `identifier` / `group` / `comment`。
   - 通过后：`docs/releases/v2.7.32.md` 验证表 + "真机结果"段、`CHANGELOG.md` 2.7.32 条目、`docs/reference/capabilities.md` 两处"真机待验"、登记表 `verified`，提交 `Record the 2.7.32 real-project rerun; ...`。
2. **阶段 3 剩余子批次**（Base 未封装 61 类型 / 194 成员，全部在 `Siemens.Engineering.Base.dll`，V20/V21 都有）：
   ...
   - ~~③-③ 用户管理与安全~~ **2.7.32 完成**（`Security` / `Umac` 功能类型归零；`ManageSyslogServers` / `ManagePasswordPolicy` / `ManageUmcUsers` + `ManagePlcCertificate` / `ManageProjectUserManagement` 扩展；真机待验）。官方页面 id（`https://docs.tia.siemens.cloud/api/khub/maps/gpR5ZkKnLSuzVoGX1ovoKg/topics/<id>/content`）："Functions for UMAC Global Users and UMC Server" 目录 `HHAe5KUz11B0PKDlSxUKhw`、Adding an UMC User `RaSbPMQe6yPvxCsBzVctNw`、Managing offline UMC User And User Groups `hymB8NODTv2Be0KT264V0Q`、Authentication `PVFJJmFJBQqKACi3rTNi1A`、Complete sample code `fZrjbp~W2nzlP1uqGHjCiw`、Setting password policies for UMAC `3MwpthV7Ol6LEUkK8tjfjQ`、for PLC `4Wov24Ut~GV88CySQ3dUVQ`、Accessing Anonymous user `C38M0Qpc12CcZF4hrTJtBQ`、Managing certificate `cSHjp~ggDchSBvrL2zgyWQ`、Managing dynamic certificate settings `4dAdhWtWlHDplZkfGghnNQ`、Managing Syslog configuration `FGn9yQvNFELGZft4BvqdOQ`、Downloading User Management data `KqHGmbijjSjYOaWIBbTUuQ`。正文搜索：`POST https://docs.tia.siemens.cloud/api/khub/clustered-search`，JSON `{"query":"...","maxCount":10}`，结果 `results[].entries[].topic.contentId`（工程级 `SyslogServerProvider` 没有专页，只有 XML 注释 "Syslog settings global on project"）。
   ...
   - 每个子批次一个发布（③-④ 为 2.7.33）；先在浏览器里把官方章节逐页读完（`https://docs.tia.siemens.cloud/r/en-us/v21/tia-portal-openness-api-for-automation-of-engineering-workflows`，`get_page_text` 可直接取正文），再对照 `TIA_V21_PublicAPI\V21\net48\Siemens.Engineering.Base.xml` 核成员签名（V21 的 `net48` 目录没有单体 `Siemens.Engineering.dll/.xml`，Base 类型在 `Siemens.Engineering.Base.*`；V20 仍是 `Siemens.Engineering.xml`）。
3. 之后按路线图：阶段 4 Step7（软件单元、`PlcDocument*`、校验和/仿真设置提供者）→ 5 经典 WinCC 文件夹层次 → 6 选件包（只做形状检查）。

剩余缺口一览（2.7.32 审计，未封装功能类型 / 成员；壳子类型不计）：

| 层次 | 剩余 | 内容 |
|---|---|---|
| 核心 | 120 / 360 | Base 52 / 161、Step7 44 / 140、经典 WinCC 24 / 59 |
...
| **合计** | **230 / 896** | 2.7.31 为 239 / 929，2.7.30 为 253 / 1,006，2.7.29 为 267 / 1,067 |
...
- 安全 / UMC（2.7.32，PublicAPI 核实，未真机）：`Security` 与 `Umac` 的功能成员 V20/V21 完全一致（V21 只多 `.Any()`）；`SyslogServerProvider` 是工程服务（`project.GetService`），`SyslogServer.Port` 是 `Int64`，`AssignedModules` 是 `DeviceItemAssociation`（有 `Add` / `Remove` / `Contains`）；`SysLogTransportProtocolType` 在 `Siemens.Engineering.HW` 而非 `.HW.Features`，`SysLogServerConfigurationComposition.Create(address, UInt16 port)`；`UmcUserGroupComposition.CreateOfflineUmcUserGroup()` 不带名字（之后 `SetName`），`UmcUser` 派生自 `User`（`Name` / `Roles`），`UmcUserGroup` 不是（自带 `Name` / `Roles` / `Description`）；`UmcServer` 没有公开标量，`Authentication` 是 `EventHandler<UmcAuthenticationEventArgs>`，凭据写进 `e.UmcCredentials.Name` / `SetPassword`；`CustomDeviceFunctionRight.Comment` 只读（只能在 `Create(name, group, comment)` 时给）；`CertificateTemplate` 五个标量全可写，`SubjectAlternativeName.Type / Value` 只读（删了重建）；`Certificate.Id` 是 `UInt32`；`PasswordPolicyConfigurator` 的数值属性都是 `Int16`。
